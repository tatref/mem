        ...
        while running {
            ...
            match cmd {
                "snap" => {
                    let now = Instant::now();
                    let (heap, stack) = {
                        let lock = ProcessLocker::lock(self.pid).unwrap();
                        let heap = lock.stack().unwrap();
                    ...
                    println!("heap.len()={}B, stack.len()={}B", heap.len(), stack.len());
                    snap = Some((heap, stack));
                },
                "search" => {
                    let val: u8 = line.split_whitespace()
                        .nth(1).unwrap()
                        .parse().unwrap();
                    for (idx, x) in snap.as_ref().unwrap().0.iter().enumerate() {
                        if *x == val {
                            println!("idx={}", idx);
                        }
                    }
